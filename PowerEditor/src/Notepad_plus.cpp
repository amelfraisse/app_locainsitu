...
Notepad_plus* Notepad_plus::_Singleton = NULL;
    ...
    {        
		if (notification->nmhdr.hwndFrom == _statusBar.getHSelf())
        {
			...
			if (lpnm->dwItemSpec == DWORD(STATUSBAR_CUR_POS))
			{
				...
				if (isFirstTime)
               changeDlgLang(_goToLineDlg.getHSelf(), _goToLineDlg.GetTagName().c_str());
			}
        }
		break;
	}
	...
	switch (id)
	{
		...
		case IDM_MACRO_RUNMULTIMACRODLG :
		{
			if (!_recordingMacro) // if we're not currently recording, then playback the recorded keystrokes
			{
				...
				if (isFirstTime)
				{
               changeDlgLang(_runMacroDlg.getHSelf(), _runMacroDlg.GetTagName().c_str());
				}
				break;
		...
		case IDM_SEARCH_REPLACE :
		{
			...
			if (isFirstTime)
            changeDlgLang(_findReplaceDlg.getHSelf(), _findReplaceDlg.GetTagName().c_str());
			break;
		}
        ...
        case IDM_EDIT_COLUMNMODE :
		{
			...
			if (isFirstTime)
         {
				changeDlgLang(_colEditorDlg.getHSelf(), _colEditorDlg.GetTagName().c_str());
         }
			break;
		}
		...
		case IDM_EXECUTE:
		{
			...
			if (isFirstTime)
            changeDlgLang(_runDlg.getHSelf(), _runDlg.GetTagName().c_str());
...
void Notepad_plus::changeMenuLang(string & pluginsTrans, string & windowTrans)
{
		...
		childNode = childNode->NextSibling("Item") )
	{
		...
		if (element->Attribute("id", &id))
		{
			...
			string sLastTrans;
			this->GetAllTranslation(MainMenuItem, "", id, ListString);
			if (ListString.empty() == false)
			{
				sLastTrans = ListString[ListString.size()-1];
				if (sLastTrans.empty() == false)
					name = sLastTrans.c_str();
			}
		...
		childNode = childNode->NextSibling("Item") )
	{
		...
		std::vector<string> ListString;
		string sLastTrans;
		this->GetAllTranslation(MainMenuSubItem, "", id, ListString);
		if (ListString.empty() == false)
		{
			sLastTrans = ListString[ListString.size()-1];
			if (sLastTrans.empty() == false)
...
{
   changeDlgLang(_preference.getHSelf(), _preference.GetTagName().c_str());

	char title[64];
	
   changeDlgLang(_preference._barsDlg.getHSelf(), _preference._barsDlg.GetTagName().c_str(), title);
	if (*title)
		_preference._ctrlTab.renameTab(_preference._barsDlg.GetTagName().c_str(), title);

   changeDlgLang(_preference._marginsDlg.getHSelf(), _preference._marginsDlg.GetTagName().c_str(), title);
	if (*title)
		_preference._ctrlTab.renameTab(_preference._marginsDlg.GetTagName().c_str(), title);

   changeDlgLang(_preference._defaultNewDocDlg.getHSelf(), _preference._defaultNewDocDlg.GetTagName().c_str(), title);
	if (*title)
		_preference._ctrlTab.renameTab(_preference._defaultNewDocDlg.GetTagName().c_str(), title);

   changeDlgLang(_preference._fileAssocDlg.getHSelf(), _preference._fileAssocDlg.GetTagName().c_str(), title);
	if (*title)
		_preference._ctrlTab.renameTab(_preference._fileAssocDlg.GetTagName().c_str(), title);

   changeDlgLang(_preference._langMenuDlg.getHSelf(), _preference._langMenuDlg.GetTagName().c_str(), title);
	if (*title)
      _preference._ctrlTab.renameTab(_preference._langMenuDlg.GetTagName().c_str(), title);

   changeDlgLang(_preference._printSettingsDlg.getHSelf(), _preference._printSettingsDlg.GetTagName().c_str(), title);
	if (*title)
		_preference._ctrlTab.renameTab(_preference._printSettingsDlg.GetTagName().c_str(), title);

   changeDlgLang(_preference._printSettings2Dlg.getHSelf(), _preference._printSettings2Dlg.GetTagName().c_str(), title);
	if (*title)
		_preference._ctrlTab.renameTab(_preference._printSettings2Dlg.GetTagName().c_str(), title);

   changeDlgLang(_preference._settingsDlg.getHSelf(), _preference._settingsDlg.GetTagName().c_str(), title);
	if (*title)
		_preference._ctrlTab.renameTab(_preference._settingsDlg.GetTagName().c_str(), title);

   changeDlgLang(_preference._backupDlg.getHSelf(), _preference._backupDlg.GetTagName().c_str(), title);
	if (*title)
		_preference._ctrlTab.renameTab(_preference._backupDlg.GetTagName().c_str(), title);
...
bool Notepad_plus::ReloadStringInUI(ResType resType, HWND hDlg, const string& dlgTagName)
{
	if (resType == DialogTitle || resType ==DialogControl)
	{
		changeDlgLang(hDlg, dlgTagName.c_str());
	}
	else if (resType == MainMenuItem || resType ==MainMenuSubItem)
	{
...
bool Notepad_plus::GetAllTranslation(ResType resType, const string &dlgTagName, UINT uItemID, std::vector<string>& ListTrans)
{
   TiXmlDocument *pLocaInSituXmlRoot = (NppParameters::getInstance())->getLocaInSituXml();
   if (pLocaInSituXmlRoot == NULL)
      return false;

   	const char *sResTypeToFind;
	const char *sResNameToFind;
	if (resType == DialogTitle || resType ==DialogControl)
	{
		sResTypeToFind = "dialog";
		sResNameToFind = dlgTagName.c_str();
	}
	else if (resType == MainMenuItem)
	{
		sResTypeToFind = "MainMenuItem";
		sResNameToFind = "";
	}
	else if (resType == MainMenuSubItem)
	{
		sResTypeToFind = "MainMenuSubItem";
		sResNameToFind = "";
	}
	else
		return false;

   TiXmlNode* pNodeGroup = NULL;
   for (TiXmlNode* pNodeExplore = pLocaInSituXmlRoot->FirstChildElement("group"); pNodeExplore ; pNodeExplore = pNodeExplore->NextSibling("group"))
   {
      TiXmlElement* pElement = pNodeExplore->ToElement();
      const char *sResType = pElement->Attribute("restype");
      const char *sResName = pElement->Attribute("resname");

      if (strcmp(sResType, sResTypeToFind) == 0 && strcmp(sResName, sResNameToFind) == 0)
      {
         pNodeGroup = pNodeExplore;
         break;
      }
   }
   ...
   if (pNodeGroup != NULL)
   {
      TiXmlNode* pTransUnitNode = NULL;
      for (TiXmlNode *childNode = pNodeGroup->FirstChildElement("trans-unit"); childNode ;	childNode = childNode->NextSibling("trans-unit") )
      {
		   TiXmlElement *element = childNode->ToElement();
		   int id;
		   const char *sentinel = element->Attribute("id", &id);
         ...
         if (id == uItemID)
         {
            pTransUnitNode = childNode;
            break;
         }
      }
   
      if (pTransUnitNode != NULL)
      {
         TiXmlNode* pNodeTarget;
         for (pNodeTarget = pTransUnitNode->FirstChildElement("target"); pNodeTarget; pNodeTarget = pNodeTarget->NextSibling("target") )
         {
            TiXmlElement* pElementTarget = pNodeTarget->ToElement();
            const char* sTranslation = pElementTarget->Attribute("name");

            if (sTranslation && sTranslation[0])
            {
               string sTrans = sTranslation;
               ListTrans.push_back(sTrans);
            }

            if (!pNodeTarget->NextSibling("target"))
               break;
         }
	   }
   }
...
bool Notepad_plus::ChangeString(ResType resType, const string &dlgTagName, UINT uItemID, string& sNewTranslation)
{
   TiXmlDocument *pLocaInSituXmlRoot = (NppParameters::getInstance())->getLocaInSituXml();
   if(!pLocaInSituXmlRoot) 
      return false;
   ...
   if (pNodeGroup == NULL)
   {
      //We create the corresponding node
      pNodeGroup =  new TiXmlElement("group");
      ((TiXmlElement*)pNodeGroup)->SetAttribute("restype", sResTypeToFind);
      ((TiXmlElement*)pNodeGroup)->SetAttribute("resname", sResNameToFind);

      //We insert it
      pNodeGroup = pLocaInSituXmlRoot->InsertEndChild(*pNodeGroup);
   }
   ...
   {
      //We create the corresponding node
      pTransUnitNode =  new TiXmlElement("trans-unit");
      ((TiXmlElement*)pTransUnitNode)->SetAttribute("id", uItemID);

      //We insert it
      pTransUnitNode = pNodeGroup->InsertEndChild(*pTransUnitNode);
   }

   TiXmlElement* pElementTarget =  new TiXmlElement("target");
   pElementTarget->SetAttribute("name", sNewTranslation.c_str());
   pTransUnitNode->InsertEndChild(*pElementTarget);
...
bool Notepad_plus::changeDlgLang(HWND hDlg, const char *dlgTagName, char *title)
{
   ...
   // Also try to find translation in LocaInSituXml
   TiXmlNode* pNodeDialog = NULL;
   TiXmlDocument *pLocaInSituXmlRoot = (NppParameters::getInstance())->getLocaInSituXml();
   if (pLocaInSituXmlRoot != NULL)
   {
      for (TiXmlNode* pNodeExplore = pLocaInSituXmlRoot->FirstChildElement("group"); pNodeExplore ; pNodeExplore = pNodeExplore->NextSibling("group"))
      {
         ...
         if (strcmp(sResType, "dialog") == 0 && strcmp(sResName, dlgTagName) == 0)
         {
            pNodeDialog = pNodeExplore;
            break;
         }
      }
   }
	...
	// Set the text of child control form LocaInSituXml
   if (pNodeDialog)
   {
      //for (TiXmlNode *childNode = pNodeDialog->FirstChildElement("trans-unit"); pNodeDialog ;	pNodeDialog = pNodeDialog->NextSibling("trans-unit") )
      for (TiXmlNode *childNode = pNodeDialog->FirstChildElement("trans-unit"); childNode ;	childNode = childNode->NextSibling("trans-unit") )
      {
		   TiXmlElement *element = childNode->ToElement();
		   int id;
		   const char *sentinel = element->Attribute("id", &id);
         ...
         TiXmlNode* pNodeTarget;
         for (pNodeTarget = childNode->FirstChildElement("target"); pNodeTarget; pNodeTarget = pNodeTarget->NextSibling("target") )
         {
            if (!pNodeTarget->NextSibling("target"))
               break;
         }
		 ...
		 if (pNodeTarget != NULL)
		 {
			 ...
			 const char* sTranslation = pElementTarget->Attribute("name");
			   ...
			   if (sentinel && (sTranslation && sTranslation[0]))
			   {
				   HWND hItem = NULL;
				if (id == 0)
				   hItem = hDlg;
				   ...
				   if (hItem)
					   ::SetWindowText(hItem, sTranslation);
			   }
			}
	   }
   }
...
bool Notepad_plus::GetSourceName(ResType resType, const string &dlgTagName, UINT uItemID, string& sSourceName)
{
	TiXmlDocument *pLocaInSituXmlRoot = (NppParameters::getInstance())->getLocaInSituXml();
	if(!pLocaInSituXmlRoot) 
	  return false;
	...
	TiXmlElement *pSourceElement = pTransUnitNode->FirstChildElement("source");
	if (pSourceElement == NULL)
	{
		return false;
	}

	const char *name = pSourceElement->Attribute("name");
...
bool Notepad_plus::SetSourceName(ResType resType, const string& dlgTagName, UINT uItemID, const string& sSourceName)
{
   TiXmlDocument *pLocaInSituXmlRoot = (NppParameters::getInstance())->getLocaInSituXml();
   if(!pLocaInSituXmlRoot) 
      return false;
   ...
   if (pTransUnitNode == NULL)
   {
	...
	TiXmlElement *pSourceEleemnt = pTransUnitNode->FirstChildElement("source");
	if (pSourceEleemnt != NULL)
	{
		if (strcmp(pSourceEleemnt->Value(), sSourceName.c_str()) != 0)
			return false;
	}
	else
	{
	   TiXmlElement* pElementTarget =  new TiXmlElement("source");
	   pElementTarget->SetAttribute("name", sSourceName.c_str());
	   pTransUnitNode->InsertEndChild(*pElementTarget);
	}
...
LRESULT Notepad_plus::runProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (Message)
	{
		...
		{
			//HMENU hMainMenu = ::GetSystemMenu(_hSelf, FALSE);
			//HMENU hMainMenuStored = _mainMenuHandle;
			HMENU hMenuRetrieved = (HMENU)lParam;
			if (hMenuRetrieved != NULL)
			{
				UINT uId = ::GetMenuItemID(hMenuRetrieved, wParam);
				if (uId != 0)
				{
					char sName[512];
					::GetMenuString(hMenuRetrieved, uId, sName, 512, MF_BYCOMMAND);

					LocaInSituData LocaData;
					ResType resType = MainMenuSubItem;

					LocaData.SetUIParams(resType, _hSelf, _sTagName, uId, sName);
					Notepad_plus::getInstance()->_LocaInSituDlg.SetLocaParam(LocaData);
					Notepad_plus::getInstance()->_LocaInSituDlg.doDialog();
				}
			}
		...
		case WM_CREATE:
		{
         ...
         _configStyleDlg.SetTagName(string("Style Configurator"));
			_preference.init(_hInst, _hSelf);
         _preference.SetTagName(string("Preference"));
			...
			//--Init dialogs--//
         _LocaInSituDlg.init(_hInst, hwnd);

         _findReplaceDlg.init(_hInst, hwnd, &_pEditView);
         _findReplaceDlg.SetTagName(string("Find"));
         ...
         _goToLineDlg.SetTagName(string("GoToLine"));

			_colEditorDlg.init(_hInst, hwnd, &_pEditView);
         _colEditorDlg.SetTagName(string("ColumnEditor"));

         _aboutDlg.init(_hInst, hwnd);

			_runDlg.init(_hInst, hwnd);
         _runDlg.SetTagName(string("Run"));

			_runMacroDlg.init(_hInst, hwnd);
         _runMacroDlg.SetTagName(string("MultiMacro"));

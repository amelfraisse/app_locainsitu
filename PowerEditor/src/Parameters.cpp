...
bool NppParameters::load()
{
   ...
   // Load xlif File for LocaInSitu Translation
   char sLocaInSituXmlPath[MAX_PATH];
   strcpy(sLocaInSituXmlPath, _nppPath);
   PathAppend(sLocaInSituXmlPath, "LocaInSitu.xml");
   _pLocaInSituXml = new TiXmlDocument(sLocaInSituXmlPath);
   loadOkay = _pLocaInSituXml->LoadFile();
	if (!loadOkay)
	{
		delete _pLocaInSituXml;
		_pLocaInSituXml = NULL;
	}	

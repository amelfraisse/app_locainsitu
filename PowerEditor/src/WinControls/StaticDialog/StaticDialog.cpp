...
#include "SysMsg.h"
#include "WinControls\LocaInSitu\LocaInSituDlg.h"
...
BOOL CALLBACK StaticDialog::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message) 
	{
		case WM_CONTEXTMENU :
		{
			...
			lPoint.x = xPos;
			lPoint.y = yPos;			
			//::ClientToScreen(_hSelf, &lPoint);

			bool bFind = false;
			int iID = 0;
			char strTry[512];

         //We initialize with window title
         ::GetWindowText(_hSelf, strTry, 512);

			HWND hWndChild = ::GetWindow(_hSelf, GW_CHILD);


			RECT minRect;
			minRect.left   = -0;
			minRect.right  = 999999;
			minRect.top    = 0;
			minRect.bottom = 999999;


			while (hWndChild)
			{
				RECT Rect;
				POINT lPointConverted = lPoint;
				
				::GetWindowRect(hWndChild, &Rect);
				//::ClientToScreen(hWndChild, &lPointConverted);

				if ( (Rect.left   < lPointConverted.x) &&
					 (Rect.right  > lPointConverted.x) &&
					 (Rect.top    < lPointConverted.y) &&
					 (Rect.bottom > lPointConverted.y) )
				{
					if ( Rect.right  < minRect.right &&
						 Rect.bottom < minRect.bottom )
					{
						minRect = Rect;
						iID = ::GetDlgCtrlID(hWndChild);
						::GetWindowText(hWndChild, strTry, 64);
					}
				}
		 ...
		 if (iID ==0)
			 ...
			 resType = DialogControl;

		 LocaData.SetUIParams(resType, _hSelf, _sTagName, iID, strTry);
